...
mxArray *gsparse_nosort(const int *ii,int imod,
			...
			void quick_sort (struct elementGnuSort *a, int n) {
			    int i, j, p;
			    struct elementGnuSort t;
			    if (n < 2)
			        return;
			    p = a[n / 2].i;
			    for (i = 0, j = n - 1;; i++, j--) {
			...
			void prefixsum_inplace(int *x, int N) {
				//stolen from: http://stackoverflow.com/questions/21352816/prefix-sums-taking-too-long-openmp
			    int *suma;
			    #pragma omp parallel
			    {
			...
			void prefixsum_inplace_mwSize(mwSize *x, int N) {
				//stolen from: http://stackoverflow.com/questions/21352816/prefix-sums-taking-too-long-openmp
			    mwSize *suma;
			    #pragma omp parallel
			    {
			...
			mxArray* gnuSort(const int *ii_in,const int *jj,
			    ...
			    int len,int M,int N,int Nzmax){
			      ...
			      struct elementGnuSort* list = NULL;
			      list = (struct elementGnuSort*) mxMalloc(len*sizeof(struct elementGnuSort));
			      ...
			      __gnu_parallel::sort(list, list+len, __gnu_parallel::multiway_mergesort_tag());
			    	...
			    	mwSize *jcS_d;
						...
						for (size_t m = 0; m < N; m++) {
							for (size_t k = 0; k < len; k++) {
								...
								else if (list[sumCol + k].j >currCol) {
									jcS_d[m] = sumCol+k;
									sumCol = sumCol + k;
									currCol++;
									break;
			        ...
			        quick_sort( &list[jcS_d[rowNr]], jcS_d[rowNr+1] - jcS_d[rowNr]  );
					  }

					  //Find the doubles!
					  int* doubles;
					  doubles = (int*) mxCalloc(N, sizeof(int));
					  ...
					  for (int rowNr = 0; rowNr < N; rowNr++) {
						...
						int doubles_sum = __gnu_parallel::accumulate(doubles, doubles+N, 0);
					  //Calculate the culmative sum of doubles.
						prefixsum_inplace(doubles,  N);
					  //Copy the values.
					  mwSize* jcS_d_old;
					  jcS_d_old = (mwSize*) mxMalloc((N+1) * sizeof(mwSize));
					  ...
					  mxArray* S = mxCreateSparse(M,N,len-doubles_sum,mxREAL);
					  ...
					  //mxFree(jcS_d);
						//return mxCreateSparse(M,N,Nzmax == -1 ? 0 : Nzmax, mxREAL);
					  return S;
...
#pragma omp barrier
  ...
  if (len == 0) {
    ...
    return;
		...
		//plhs[0] = mxCreateSparse(M,N,Nzmax == -1 ? 0 : Nzmax, mxREAL);
		//printf("DOOOOOOOOOOOOOOOOONE!\n");
		return;
	}
