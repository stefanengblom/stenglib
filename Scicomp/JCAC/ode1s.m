...
%
%   mass         Matrix {[]}            Constant mass-matrix. 
%
%   jpattern     Matrix {[]}            Jacobian-sparsity-pattern-matrix. 
%
...
% JC: Comments by Juan Carlos Araujo Cabarcas

out = struct('nfun',0,'njac',0,'nNit',0,'nLU',0,'nlinsolve',0, ...
    'nStpAccepted',0,'nFailIntTol',0);
...
optdef.atol = 1e-6;
optdef.rtol = 1e-4;
optdef.mass = [];                                           % JC: global mass (diagonal tested);
optdef.jpattern = [];                                                             % JC: global mass (diagonal tested);
...
optdef.nlinauto = 1;
optdef.reportfun = [];
optdef.info = 0;                                                           % JC: 0: no info, 1: show disp, 2: show warn
...
mass = opts.mass;                                                          % JC: what format is adequate?
...
% cases for the Jacobian-Sparsity-Pattern
if isempty(jpattern)
    if isempty(mass) % full case
        mass = speye( size(yy,1) );        % should be full
        % mass = eye( size(yy,1) );
    else 
        jpattern = mass;
        warning('Jpattern is assigned to have the same sparsity pattern as the mass (identity?) matrix.');
    end
else
    if isempty(mass)
        mass = speye( size(yy,1) );
    end
end
...
if nlinauto == 1
  ...
  nlinopts.thetaREF = 0.4;        % **JC 0.4 b4 target convergence factor
  nlinopts.thetaJAC = 0.4;        % **JC 0.4 b4 recompute Jacobian
  nlinopts.thetaLU = 0.4;         % **JC 0.4 b4 refactorize Jacobian
...
tt = tspan(1);
tend = tspan(end);
[h,TOL,out] = l_initialstep(rtol,atol,adaptivity,order, ...
                     NM,errnorm1,errnorm2,tt,yy,fun,mass,jpattern,out,varargin{:}); % ***mass
...
while 1
  ...
  [y3,rhon,out] = l_step(true,tt,yy,h,[], ...
                     TOL,NM,errnorm1,errnorm2,nlinopts,fun,mass,jpattern,out,varargin{:});
  if isempty(y3)
  ...
  if nlinauto < 3
    ...
    [y1,rhon,out] = l_step(false,tt,yy,h2,0.5*(yy+y3), ...
                       TOL,NM,errnorm1,errnorm2,nlinopts,fun,mass,jpattern,out,varargin{:});
  else
    % be more suspicious...                %** what is []?
    [y1,rhon,out] = l_step(false,tt,yy,h2,[], ...
                       TOL,NM,errnorm1,errnorm2,nlinopts,fun,mass,jpattern,out,varargin{:});
  end
  if isempty(y1)
  ...
  if nlinauto == 1
    ...
    [y2,rhon,out] = l_step(false,tt+h2,y1,h2,y1+0.5*(y3-yy), ...
                       TOL,NM,errnorm1,errnorm2,nlinopts,fun,mass,jpattern,out,varargin{:}); % mass
  elseif nlinauto == 2
    % using only the full step
    [y2,rhon,out] = l_step(false,tt+h2,y1,h2,y3, ...
                       TOL,NM,errnorm1,errnorm2,nlinopts,fun,mass,jpattern,out,varargin{:}); % mass
  else
    % suspicious...
    [y2,rhon,out] = l_step(false,tt+h2,y1,h2,[], ...
                       TOL,NM,errnorm1,errnorm2,nlinopts,fun,mass,jpattern,out,varargin{:}); % mass
  end
  if isempty(y2)
    h2 = l_limiter(rhon,1)*h2;
  ...
  if rho >= rhomin                                                         % JC: get the time step from here
    % step accepted: record the value of T_2(h/2)
    out.nStpAccepted = out.nStpAccepted + 1;
    
    %fprintf('-----step accepted %d\n',out.nStpAccepted);                  % JC:
    ...
    t(i) = tt;
    y(:,i) = yy;
    status = reportfun(tt,yy,[],varargin{:});                              % JC: is this calling fun?
    ...
    elseif status ~= 0
      ...
      status = reportfun(tt,yy,'done',varargin{:});                        % JC: is this calling fun?
      t = t(1:i);
      y = y(:,1:i);
      break;
    end
    ...
    out.nFailIntTol = out.nFailIntTol + 1;
  end
  h = rho*h;
...
if nargout < 2
  t = struct('x',t,'y',y,'solver','ode1s', ...
      'nStpAccepted',out.nStpAccepted,'nFailIntTol',out.nFailIntTol, ...
      'nfun',out.nfun,'njac',out.njac,'nNit',out.nNit,'nLU',out.nLU, ... 
      'nlinsolve',out.nlinsolve);
end

%JC: info, level of information 
fprintf('\n nStpAccepted = %d, nFailIntTol = %d, nfun = %d, njac = %d, nNit = %d, nLU = %d, it/stp = %f \n',...
    out.nStpAccepted,out.nFailIntTol,out.nfun,out.njac,out.nNit,out.nLU, out.nNit/out.nStpAccepted);


%-------------------------------------------------------------------------
function [h,TOL,out] = l_initialstep(rtol,atol,adaptivity,order, ...
                                  NM,errnorm1,errnorm2,t0,y0,fun,mass,S, ...
                                  out,varargin) % mass
...
out.nfun = out.nfun+1; % JC: function call
dydt0 = dydt+dydtexpl;
...
%-------------------------------------------------------------------------
function [y,rhon,out] = l_step(fullstep,t0,y0,h,yest,TOL, ...
                           NM,errnorm1,errnorm2, ...
                           nlinopts,fun,mass,S,out,varargin) % JC: mass, S: jpattern
...
[ydelta,rhon,out] = l_implsolve(fullstep,ydelta,t0,h,y0,TOL, ...
                            NM,errnorm1,errnorm2, ...
                            nlinopts,fun,mass,S,out,varargin{:}); % mass
if isempty(ydelta)
...
%-------------------------------------------------------------------------
function [ydelta,rhon,out] = l_implsolve(fullstep,ydelta,t0,h,y0,TOL, ...
                                     NM,errnorm1,errnorm2, ...
                                     nlinopts,fun,mass,S,out,varargin) % JC: mass, S: jpattern
...
%
% Reference:
%   [1] K. Gustafsson and G. S�derlind: "Control Strategies for the
...
%persistent J wksp tJAC L U P hLU eta theta L2 U2 P2;                      % JC: original line
persistent J wksp wksp2 tJAC L U P Q hLU eta theta L2 U2 P2 Q2;            % JC: S: Sparsity pattern
...
eta = max(eta,eps); % convergence factor (reused on success)
if fullstep
  eta = eta^nlinopts.etapow;
end
kappa = nlinopts.kappa;             % extra accuracy
maxiter = nlinopts.maxiter;         % max number of iterations

% The equation solved is nowspeye(size(mass))
%
%   ydelta = h*F(t0,y0)+h*G(t0+h,y0+ydelta,[t0 y0]),                       % JC: Mass matrix?
%
...
if fullstep
  ...
  if isempty(J) || theta*hLU-abs(h-hLU) > nlinopts.thetaJAC*hLU
    ...
    %[J,wksp,~,nfun] = numjac(fun,t0+h,y0+ydelta,dydt,{0 1},wksp,0,[],[], ... % original line
    %                  t0,y0,varargin{:});
    
    if isempty(S) % JC: cases empty S and sparse mass
        [J,wksp,~,nfun] = numjac(fun,t0+h,y0+ydelta,dydt,{0 1},wksp,0,[],[], ... 
                      t0,y0,varargin{:});
       if spM
            J = sparse(J);
       end          
        ...
        if spM
            [J,wksp,wksp2,nfun] = numjac(fun,t0+h,y0+ydelta,dydt,{0 1},wksp,0,S,wksp2, ... 
                      t0,y0,varargin{:});
        else
            [J,wksp,~,nfun] = numjac(fun,t0+h,y0+ydelta,dydt,{0 1},wksp,0,[],[], ... 
                      t0,y0,varargin{:});
    ...
    out.nfun = out.nfun+nfun;
    out.njac = out.njac+1;
    % *** user-defined Jacobian (exact and numerical)
    tJAC = t0; % (formed with data available up to t = t0)
    
    %[L,U,P] = lu(eye(size(J))-h*J); % *** J sparse! % ***JC original line
    if spM 
        %issparse(mass - h*J)
        [L,U,P,Q] = lu( mass - h*J); % *** J sparse, mass included     % JC: LUP
                               % as identity first
    else
        [L,U,P] = lu( mass - h*J); % *** J sparse, mass included     % JC: LUP
                               % as identity first
    end
    ...
    hLU = h;
    P2 = []; % force refactorization at the half-step                      % JC: change for original P2 instead 
  %elseif isempty(P) || abs(h-hLU) > nlinopts.thetaLU*hLU                  % **JC: original line?
  elseif isempty(P) || abs(h-hLU) > nlinopts.thetaLU*hLU                   % JC: isempty(p), the same? how to check?
    ...
    out.nLU = out.nLU+1;
    hLU = h;
    P2 = [];                                                               % JC: change for original P2 instead 
  end 
else
  % when taking a half-step, the previous full step always
  % converged and hence we only care about the factorization
  % if isempty(P2)                                                         % **JC: original line?
  if isempty(P2)                                                           % JC: isempty(p), the same? how to check?
    %[L2,U2,P2] = lu(eye(size(J))-h*J);                                    % *** J sparse, JC: original line!
    if spM
        [L2,U2,P2,Q2] = lu( mass - h*J); % *** J sparse, mass included   % JC: LUP 
                               % as identity first
    else
        [L2,U2,P2] = lu( mass - h*J); % *** J sparse, mass included   % JC: LUP 
                               % as identity first
    end
...
% first Newton-step
% res = ydelta-h*dydtexpl-h*dydt;                                          % JC: original line
res = mass*ydelta-h*dydtexpl-h*dydt;                                       % JC: mass here?
if fullstep
  % delta = U\(L\(P*(res)));                                               % JC: original line
  if spM
    delta = Q*( U\(L\( P*res )) );  
  else
    delta = U\(L\( P*res ));                                                % JC: LUP
  end
  out.nlinsolve = out.nlinsolve + 1;
else
  % delta = U2\(L2\(P2*res));                                              % JC: original line
  if spM
    delta = Q2*( U2\(L2\(P2*res)) );   
  else
    delta = U2\( L2\(P2*res) );                                               % JC: LUP
  end
  out.nlinsolve = out.nlinsolve + 1;
end
ydelta = ydelta-delta;
...
for i = 2:maxiter
  ...
  %res =  ydelta-h*dydtexpl-h*fun(t0+h,y0+ydelta,t0,y0,varargin{:});       % JC: original line
  res =  mass*ydelta-h*dydtexpl-h*fun(t0+h,y0+ydelta,t0,y0,varargin{:});   % JC: mass here?
...
if fullstep
  ...
  if tJAC ~= t0
    ...
    P = []; % force refactorization                                        % JC: still works? (non-capital p)
  end
end
...
%
% Reference:
%   [1] G. S�derlind, L. Wang: "Adaptive time-stepping and
...
%
% Reference:
%   [1] G. S�derlind: "Digital Filters in Adaptive Time-Stepping",
